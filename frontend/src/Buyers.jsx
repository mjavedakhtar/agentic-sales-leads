...
const STATUS={
  ...
  material_user_or_specifier:{label:'Platform user / specifier',tone:'purple'},
  downstream_buyer:{label:'Turnkey solution buyer',tone:'neutral'},
  unclear:{label:'Buying responsibility unclear',tone:'amber'},
};
const ROLES=[['material_use','Deploys the software'],['specification','Specifies the platform architecture'],['procurement','Purchases commercial licenses'],['finished_components','Purchases turnkey solutions'],['customer_supplied','Uses customer-supplied software']];
...
export function Buyers({id,example=false,data,navigate}){
  ...
  const filters=[['all','All companies'],['supported_buyer','Supported software buyers'],['material_user_or_specifier','Users / specifiers'],['downstream_buyer','Turnkey solution buyers'],['unclear','Unclear']];
  ...
  return <div className="buyer-page">
    ...
    {initialReady&&<>
      ...
      <div className="buyer-filters" role="group" aria-label="Filter buying responsibility">{filters.map(([key,label])=><button key={key} aria-pressed={filter===key} title={key==='unclear'?'Purchasing responsibility is unknown. These companies may also be material users or specifiers.':undefined} onClick={()=>setFilter(key)}>{label}<span>{counts[key]}</span></button>)}</div>
      ...
      {!visible.length&&<Empty icon="shield" title={filter==='supported_buyer'?'No supported material buyer in this view.':'No companies match this filter.'}>{filter==='supported_buyer'?'The available evidence does not establish purchasing responsibility. Relevant prospects can still be reviewed for a research or influence opportunity.':'Choose another responsibility to explore the findings.'}</Empty>}
      <div className="buyer-boundary"><Icon name="people" size={18}/><span>Public evidence can support responsibility for purchasing a material category. It does not establish current buying intent, available budget, or a named decision maker. Human qualification remains the next step.</span></div>
    </>}
