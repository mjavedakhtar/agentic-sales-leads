...
export function Research({id,data,navigate,refresh}){
  ...
  return <>
    ...
    {run.status==='researching'?<ResearchProgress run={run} navigate={navigate}/>:run.status==='failed'?<section className="panel research-failure">
      ...
      <div className="insight-strip"><span className="icon-tile"><Icon name="info"/></span><div><strong>Fit is a hypothesis. Evidence tells you what to ask next.</strong><p>A high score does not establish technical qualification, buying responsibility or material demand. Open an assessment to see the gaps.</p></div></div>
    </>}
  </>
}
