...
const playDesigns=[{icon:'factory',type:'AUTOMOTIVE',title:'Find the next assembly opportunity',desc:'Automotive plants with predictive-maintenance ICP, then hiring and program signals.',tone:'lilac'},{icon:'bolt',type:'ELECTRONICS',title:'Automate quality assurance',desc:'Robotics manufacturers, then stack fit, intent, and who licenses the platform.',tone:'peach'},{icon:'layers',type:'INDUSTRIAL EDGE',title:'Look beyond the usual accounts',desc:'Industrial IoT ICP, then deployment mismatch and the actual license buyer.',tone:'mint'}];
  ...
  return <>
    ...
    <section className="research-composer panel" aria-label="Start a research brief">
      ...
      <form onSubmit={e=>{e.preventDefault();start()}}>
        <textarea aria-label="Research brief" aria-describedby="research-mode-description" value={query} onChange={e=>setQuery(e.target.value)} placeholder={live?'Find German automotive assembly plants for CloudScale AI predictive maintenance, or explore another market...':'Find German automotive assembly plants for CloudScale AI predictive maintenance.'} maxLength={1200} rows={2} required disabled={busy}/>
