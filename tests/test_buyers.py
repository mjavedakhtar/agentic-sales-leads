...
SCOPE = {'product_id': 'DS-PRO', 'product_name': 'DataStream Pro',
         'geography': 'Germany', 'application': 'Factory-floor edge telemetry',
         'additional_constraints': ['Exclude distributors.']}
QUOTE = ('Delta Components IT/OT purchasing owns commercial licenses for the edge '
         'telemetry platform at its operating German plant.')


def candidate(**overrides):
    return dict(id='delta', name='Delta Components', country='Germany', domain='example.com',
                application='Industrial edge telemetry', score=72, score_upper=92,
...
def citation(item=None, **overrides):
    ...
    result = dict(source_id=item['id'], quote=item['text'], entity_matches=True,
                  ...
                  reason='The quoted source explicitly assigns platform licensing to this business.')
    result.update(overrides)
    return result


def role(status='unknown', evidence=None):
    return dict(status=status, reason='Responsibility is assessed for this specific software category and operating entity.',
                evidence=evidence or [])


def judgment(candidate_id='delta', **roles):
    result = dict(candidate_id=candidate_id, **{key: role() for key in ROLE_KEYS},
                  contradictions=[], summary='Software procurement is evaluated separately from commercial fit.',
                  next_search_question='Who owns platform licensing at this manufacturing site?')
    result.update(roles)
...
@pytest.mark.parametrize('language,quote', [
    ('en', QUOTE),
    ('de', 'Delta Components Einkauf lizenziert die Edge-Telemetry-Plattform fuer die eigene Fertigung in Deutschland.'),
])
...
@pytest.mark.parametrize('kind,expected', [
    ('turnkey_delivery', 'turnkey_buyer'),
    ('platform_use', 'user_or_specifier'),
    ('specification', 'user_or_specifier'),
])
def test_other_responsibilities_cannot_substitute_for_software_procurement(kind, expected):
...
def test_customer_mandated_software_retains_platform_user_but_not_license_buyer():
    original = source(text='Delta Components deploys edge telemetry using software licensed and supplied by its OEM customer. Its contract excludes platform procurement.')
    proof = [citation(original)]
    assessment = judgment(platform_use=role('supported', proof), customer_mandated=role('supported', proof),
                          procurement=role('contradicted', proof))
    result = qualify_buyer(candidate(), assessment, [original])
    assert result['buyer_status'] == 'user_or_specifier'
    assert result['roles']['procurement']['status'] == 'contradicted'
    assert result['eligible'] is False
    assert followup_questions([result]) == []


def test_conflicting_purchasing_and_customer_mandated_roles_cannot_qualify():
    assessment = supported_judgment()
    assessment['customer_mandated'] = role('supported', [citation()])
    ...
    assert any('customer-mandated' in issue for issue in result['validation_issues'])


def test_unresolved_source_conflict_overrides_positive_procurement_judgment():
    assessment = supported_judgment()
    assessment['contradictions'] = ['A current contract states that the OEM customer supplies the platform license at the same site.']
    result = qualify_buyer(candidate(), assessment, [source()])
    assert result['eligible'] is False
...
@pytest.mark.parametrize('field', ['entity_matches', 'category_matches', 'current_applicable', 'relation_matches'])
...
def test_summary_only_platform_use_is_tentative_and_cannot_establish_purchasing(kind):
    original = source(source_type=kind)
    proof = [citation(original, quote='')]
    result = qualify_buyer(candidate(), judgment(platform_use=role('supported', proof),
                                                procurement=role('supported', proof)), [original])
    assert result['buyer_status'] == 'user_or_specifier' and result['eligible'] is False
    assert result['roles']['platform_use']['status'] == 'supported'
    assert result['roles']['procurement']['status'] == 'unknown'
    assert result['roles']['platform_use']['evidence'][0]['original_quote_matched'] is False
...
def test_preparing_comparison_preserves_history_and_cannot_promote_legacy_claim_cards():
    ...
    lead['evidence'] = [dict(id='legacy-fact', url='https://example.com/delta', title='Saved source',
                             quote=QUOTE, claim='The company purchases the edge telemetry platform.',
...
def test_offline_illustration_explains_same_candidates_then_additional_evidence_without_api_calls():
    ...
    assert final[0]['buyer_status'] == 'turnkey_buyer'
    assert final[1]['roles']['customer_mandated']['status'] == 'supported'
...
def test_followup_targets_unknown_buyers_and_actual_upstream_buyers_of_downstream_prospects():
    ...
    downstream = qualify_buyer(candidate(), judgment(turnkey_delivery=role('supported', [citation()]),
...
def test_followup_search_preserves_scope_and_records_new_sources_separately():
    requests = []
    grounding = {'webSearchQueries': ['Delta Components edge telemetry licensing Germany'],
                 'groundingChunks': [{'web': {'uri': 'https://example.com/delta', 'title': 'Delta Components'}}],
                 'groundingSupports': [{'segment': {'text': 'Delta Components licenses the edge telemetry platform.'},
                                        'groundingChunkIndices': [0]}]}
    def handler(request):
        requests.append(json.loads(request.content))
        return httpx.Response(200, json=response('Delta Components licenses the edge telemetry platform.', grounding=grounding))
    ...
    result = adapter.research_buyers(SCOPE, [candidate()], [source()],
                                     [{'candidate_id': 'delta', 'company': 'Delta Components',
                                       'question': 'Who licenses the edge telemetry platform for the German plant?'}])
    assert SCOPE == before
    ...
    assert 'Who licenses the edge telemetry platform for the German plant?' in prompt
...
def discovered_payload(original, *, scope_matches=True, procurement=True):
    ...
    return {'assessments': [judgment()], 'discovered_buyers': [
        dict(name='New Plant Integrator', country='Germany', domain='example.com', scope_matches=scope_matches,
...
def test_new_actual_buyers_need_followup_scope_evidence_and_supported_procurement(phase, scope_matches, procurement, expected_count):
    text = 'New Plant Integrator purchases commercial licenses for the edge telemetry platform at its operating German plant.'
    ...
    if expected_count == 2:
        ...
        assert found['name'] == 'New Plant Integrator' and found['eligible'] is True
